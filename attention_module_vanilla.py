...
class TransformerBlock(snt.Module):
  ...
  def __call__(self, inputs: tf.Tensor, training: bool) -> tf.Tensor:
    ...
    x = self.mha(x, is_training=training)
    x = self.mha_dropout(x, is_training=training)
    x += inputs  # Residual
    mha_output = x
    ...
    x = self.mlp_dropout1(x, is_training=training)
    x = tf.nn.relu(x)
    x = self.mlp_linear2(x)
    x = self.mlp_dropout2(x, is_training=training)
    return x + mha_output
